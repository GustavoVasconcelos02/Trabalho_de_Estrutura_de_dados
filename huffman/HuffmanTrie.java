...
public class HuffmanTrie {
	...
	private void buildTrie(Map<Character, Integer> frequencies) {
		...
		PriorityQueue<HuffmanTrieNode> priority = new PriorityQueue<HuffmanTrieNode>();
		
		// Itera sobre a tabela de frequências e cria um nó folha para cada símbolo
		for(Map.Entry<Character, Integer> entry : frequencies.entrySet()) {
			HuffmanTrieNode newNode = new HuffmanTrieNode(entry.getKey(), entry.getValue());
		...
		while(priority.size() > 1) {
			...
			HuffmanTrieNode left = priority.poll();
			HuffmanTrieNode right = priority.poll();
			HuffmanTrieNode newNode = new HuffmanTrieNode(left.frequency + right.frequency, 
											left, 
											right);
	...
	private void generateHuffmanCodes(HuffmanTrieNode node, String code) {
	...
	public String decompress(String compressed) { // 00110101
		...
		HuffmanTrieNode current = this.root;
		char currentBit;
